...
const Reserve = ({ params }: { params: { event_id: string } }) => {
	...
	const handleRemovePerson = (deleteIndex: number) => {
        const filterPersons = persons.filter((item, index) => deleteIndex !== index);
        setPersons(filterPersons);
    };
	return (
		<div className="d-flex justify-content-center align-items-start w-100" style={{ paddingTop: '2rem', paddingBottom: '2rem' }}>
				...
				<Card.Body>
					<Form>
						{persons.map((person, index) => (
							<>
                            <CloseButton onClick={() => handleRemovePerson(index)} />
						...
						<div className="d-flex justify-content-center">
							<Button variant="danger" onClick={handleAddPerson} style={{ marginRight: '10px', backgroundColor: '#A21D22' }}>
								Add Person
							</Button>
							<Button variant="danger" onClick={handleConfirm} style={{ marginRight: '10px', backgroundColor: '#A21D22' }}>
								Confirm
							</Button>
						</div>
					</Form>
